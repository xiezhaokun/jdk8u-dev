...
Node* MemNode::can_see_stored_value(Node* st, PhaseTransform* phase) const {
  ...
  bool skip_membars = skip_through_membars(atp, tp, phase->C->eliminate_boxing());
  if (method_matches && c2_csv_env_on("C2_CSV_TRACE_SKIP") && c2_csv_take_log_budget()) {
    tty->print("C2_CSV skip-check");
    c2_csv_print_compile(phase);
    tty->print(" skip=%d eliminate_boxing=%d atp=%d tp=%d field=%d final=%d ld_alloc=%d",
               skip_membars ? 1 : 0,
               phase->C->eliminate_boxing() ? 1 : 0,
               atp == NULL ? 0 : 1,
               tp == NULL ? 0 : 1,
               (atp != NULL && atp->field() != NULL) ? 1 : 0,
               (atp != NULL && !atp->is_rewritable()) ? 1 : 0,
               ld_alloc == NULL ? 0 : 1);
  ...
  if (skip_membars) {
    ...
    while (current->is_Proj()) {
      ...
      if (allowed_membar) {
        ...
        Node* mem = current->in(0)->in(TypeFunc::Memory);
        tty->print("C2_CSV break-opc");
        c2_csv_print_compile(phase);
        tty->print(" step=%d opc=%d final=%d", seen_count, opc, final ? 1 : 0);
        c2_csv_print_node("current", current);
        c2_csv_print_node("holder", current->in(0));
        c2_csv_print_node("mem", mem);
        c2_csv_print_proj_holder("mem", mem);
        c2_csv_print_inputs("holder", current->in(0));
        c2_csv_print_inputs("mem", mem);
        tty->cr();
      }
      break;
    }
    if (log_loop && current != NULL && !current->is_Proj()) {
      tty->print("C2_CSV break-non-proj");
      c2_csv_print_compile(phase);
      tty->print(" steps=%d", seen_count);
      c2_csv_print_node("current", current);
      c2_csv_print_proj_holder("current", current);
      c2_csv_print_inputs("current", current);
      tty->cr();
    }
      if (result != NULL) {
        st = result;
